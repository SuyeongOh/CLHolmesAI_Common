...
def save_results_to_excel(results: dict, output_path: str = "model_comparison.xlsx"):
    # 추출할 라벨 순서
    labels = ['N', 'S', 'V', 'Q', 'A', 'U']
    metrics = ['F1', 'Se', 'PPV']
    rows = []
...
def evaluate_result(result_class, target_label, beat_class_map):
    """
...
def run(np_data: dict, raw_fs: dict = None):
    """
    ...
    for model_name in model_list:
        ...
        if model_name == 'beat_classifer':
            ...
            if raw_fs is None:
                raise ValueError("resxtranformer 모델 평가에는 raw_fs가 필요합니다.")
        ...
        for pid in pid_list:
            ...
            for beat in target_data:
                resample_signal.append(preprocess_func(beat))
            ...
            sensitivity, ppv, fn = evaluate_result(result_class, target_label, beat_class_map)
