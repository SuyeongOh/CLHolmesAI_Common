...
class CELoss(nn.modules.loss._WeightedLoss):
    def __init__(self, weight=None):
        ...
        # weight = torch.tensor([1.0 / 102052, 1.0 / 200, 1.0 / 70929, 1.0 / 4140], dtype=torch.float32).to(device)
        ############
...
# class CELoss(nn.modules.loss._WeightedLoss):
#     def __init__(self, weight=None, alpha=[0.25, 0.75, 0.25, 0.75], gamma=2, use_focal=False):
#         super().__init__()
#         self.CE = nn.CrossEntropyLoss(weight=weight)
#         self.alpha = alpha  # Focal Loss의 클래스별 alpha 값 리스트
#         self.gamma = gamma  # Focal Loss의 gamma
#         self.use_focal = use_focal  # Focal Loss 사용 여부 플래그

#     def forward(self, input, target):
#         # input: [batch_size, num_classes], target: [batch_size]
#         input = input.reshape(-1, input.shape[-1])  # [batch_size, num_classes]로 재구성
#         target = target.flatten().long()  # [batch_size]로 변환
...
#         # Focal Loss 적용 여부 확인
#         if self.use_focal:
#             # Cross Entropy Loss 기반으로 Focal Loss 적용
#             pt = torch.exp(-ce_loss)  # pt = exp(-CrossEntropyLoss)

#             # 각 샘플의 클래스에 맞는 alpha 값 선택
#             alpha_t = torch.tensor([self.alpha[t] for t in target]).to(input.device)  # target에 따른 alpha 값 적용

#             # Focal Loss 계산
#             focal_loss = alpha_t * (1 - pt) ** self.gamma * ce_loss
#             return focal_loss.mean()  # Focal Loss 반환
#         else:
#             return ce_loss  # 기본 CrossEntropyLoss 반환
...
class SigFormer_pretrain(nn.Module):
    def __init__(self, patch_size=patch_size, in_chans=1, out_chans=1, num_classes=0,
        ...
        self.decoder = UpSamplePixelShuffle(f_dim=embed_dim * 2 ** (len(depths) - 1), out_channels=out_chans,
                                            upscale=patch_size * 2 ** (len(depths) - 1))
        # self.decoder = UpSamplePixelShuffle(f_dim=embed_dim*2**(len(depths)-1), out_channels=4, upscale=32)
...
class SoftmaxClassifier(nn.Module):
    def __init__(self, input_dim=2688, num_classes=4):
        super(SoftmaxClassifier, self).__init__()
        # Fully connected layer 정의: 2688차원 입력 -> 4개의 클래스 출력
        self.fc = nn.Linear(in_features=input_dim, out_features=num_classes)
        # Softmax 정의
        self.softmax = nn.Softmax(dim=1)

    def forward(self, x):
        # 입력 크기가 [128, 2688]일 경우 차원 변환 없이 그대로 처리
        # Fully connected layer를 통해 logits 계산
        logits = self.fc(x)
        # Softmax 적용하여 각 클래스의 확률 계산
...
class SigFormer_segmentation_rhythm(nn.Module):
    def __init__(self, patch_size=patch_size, in_chans=1, out_chans=1, num_classes=0,
        ...
        self.decoder = UpSamplePixelShuffle(f_dim=embed_dim * 2 ** (len(depths) - 1), out_channels=out_chans,
                                            upscale=patch_size * 2 ** (len(depths) - 1))
        # self.decoder = UpSamplePixelShuffle(f_dim=embed_dim*2**(len(depths)-1), out_channels=4, upscale= 32)#2**(len(depths)-1))
        # print(f'f_dim = {embed_dim}*2**{len(depths)-1} = {embed_dim*2**(len(depths)-1)}, depths = {len(depths)}')
        # print(f'out_chans = {out_chans}')
        # print(f'up scale = {2**(len(depths)-1)}')
    ...
    def forward_features(self, x, mask=None):
        # print(x.shape)
        # x = x.reshape(128,1,2688)
        out, features = self.encoder(x.tanh(), mask)
        # print(f'after encoder {out.shape}')
        seg = self.decoder(out)
        # print(f'after decoder {seg.shape}')
        seg = seg.squeeze(1)
        # print(seg.shape)
        seg = self.relu(self.fc1(seg))
        # print(seg.shape)
        seg = self.relu(self.fc2(seg))
        # print(seg.shape)
        seg = self.fc3(seg)
        # print(seg.shape)
    ...
    def calculate_loss(self, true, pred):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        # class_weights = torch.tensor([1.0 / 114011, 1.0 / 200, 1.0 / 139730, 1.0 / 9889], dtype=torch.float32).to(device)
        # criterion = CELoss(weight=class_weights, alpha=[0.25, 0.75, 0.25, 0.75], gamma=2, use_focal=True)

        criterion = CELoss()
        # # print(f'{pred.shape}')
        # pred = pred.reshape(-1, pred.shape[-1]).transpose(0,1)
        # print(f'{pred.shape}')
        # print(f' pred x : {(pred.reshape(-1, pred.shape[-1])).shape}')
        # print(f'gt x : {(true.flatten().long()).shape}')
        loss = criterion(pred, true.flatten().long())
        return loss
...
class two_class_SigFormer_segmentation_rhythm(nn.Module):
    def __init__(self, patch_size=patch_size, in_chans=1, out_chans=1, num_classes=0,
                 embed_dim=embed_dim, depths=depths, num_heads=num_heads,
                 window_size=window_size, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=drop_path_rate,
    ...
    def calculate_loss(self, true, pred):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        # class_weights = torch.tensor([1.0 / 102052, 1.0 / 200, 1.0 / 70929, 1.0 / 4140], dtype=torch.float32).to(device)
        # criterion = CELoss(weight=class_weights, alpha=[0.25, 0.75, 0.25, 0.5], gamma=2, use_focal=True)
        # criterion = CELoss(weight=class_weights, alpha=0.25, gamma=2, use_focal=True)
        criterion = CELoss()
        # # print(f'{pred.shape}')
        # pred = pred.reshape(-1, pred.shape[-1]).transpose(0,1)
        # print(f'{pred.shape}')
